...
for n in range(5000):
    ...
    for n in range(bits): # there must be a more efficient way to do this
        state += str(
            #((~int(seed[p])+2)&int(seed[q])&int(seed[r]))^int(seed[q])^(~int(seed[r])+2) # boolean
            (1+int(seed[q])+int(seed[r])+int(seed[q])*int(seed[r])+int(seed[p])*int(seed[q])*int(seed[r])) % 2 # algebraic
        ) # rule 145
        p = (p + 1) % bits
        q = (q + 1) % bits
        r = (r + 1) % bits
