...
const key = new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
const iv = new Uint8Array([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])

let trivium = new Trivium(key, iv)

let pt = "foo, bar, baz, qux, quux, corge, grault, garply, waldo, fred, plugh, xyzzy, and thud."
console.log("pt:", pt)

let ptData = new TextEncoder().encode(pt)
let ctData = trivium.encrypt(ptData)
console.log("enc:", Buffer.from(ctData).toString("base64"))

trivium = new Trivium(key, iv)
ptData = trivium.decrypt(ctData)
pt = new TextDecoder().decode(ptData)
